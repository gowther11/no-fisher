...
def getAllli(url):
    ...
    html = str(response.read(), encoding='utf-8').replace("&quot;", "\"").replace("\\", "").replace("<br>",
                                                                                                    "")  # 替换引号、转义符、换行符
    if not html:
        return None
...
def getData(start, count, log=False, interval = 0.5):
    data = []
    start = 7677197468
...
if __name__ == '__main__':
    datas = getData(7677197468, 10, True, interval = 0.5)
    #print(datas)

    json_str = json.dumps({'datas' : datas}, ensure_ascii=False)# 将字典装化为json串
    file = open("your_file_path.txt", 'a')
    file.write(json_str+'\n')
    #print(getAllli("https://tieba.baidu.com/p/7677197468"))